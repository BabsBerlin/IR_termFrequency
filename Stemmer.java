...
class Stemmer {
	 ...
	 */
	/*
	 * public static void main(String[] args) { char[] w = new char[501]; Stemmer s
	 * = new Stemmer(); for (int i = 0; i < args.length; i++) try { FileInputStream
	 * in = new FileInputStream(args[i]);
	 * 
	 * try { while(true)
	 * 
	 * { int ch = in.read(); if (Character.isLetter((char) ch)) { int j = 0;
	 * while(true) { ch = Character.toLowerCase((char) ch); w[j] = (char) ch; if (j
	 * < 500) j++; ch = in.read(); if (!Character.isLetter((char) ch)) { to test
	 * add(char ch) for (int c = 0; c < j; c++) s.add(w[c]);
	 * 
